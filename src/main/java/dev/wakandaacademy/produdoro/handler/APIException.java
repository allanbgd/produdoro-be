...
public class APIException extends RuntimeException {
    ...
    //construtor que tem 3 parâmetros

    private APIException(HttpStatus statusException, String message, Exception e){
        super(message, e);
        this.statusException = statusException;
        //o Error Api response vai ser instanciado num constutor desse API exception, usando .message
        //e o .description
        this.bodyException = ErrorApiResponse.builder()
                .message(message)
                // descrição do erro acontecido
                .description(getDescription(e))
                .build();

    }
    //atributos que vão ser passados pelo getter
    //exceção de negócio
    ...
    //exceção "catastrófica", com funcionalidade pra ser mapeada e resolvida
    ...
    private String getDescription(Exception e) {
        //optional que verifica se é null
        return Optional.ofNullable(e)
                .map(APIException :: getMessageCause).orElse(null);
    }
    //é static porque apenas o getDescription vai usar ele
    ...
    public ResponseEntity<ErrorApiResponse> buildErrorResponseEntity() {
        return ResponseEntity.status(statusException)
                .body(bodyException);
